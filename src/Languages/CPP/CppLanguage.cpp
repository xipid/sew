...
static bool ensureWasiSdk() {
  ...
  fprintf(
      stderr,
      "WASI SDK toolchain not found at %s. Please run sew CLI to install it.\n",
      sdkDir.c_str());
  return false;
}
...
static String getKindString(const String &typeStr) {
  if (typeStr == "int" || typeStr == "int32_t" || typeStr == "int64_t" ||
      typeStr == "unsigned" || typeStr == "size_t" || typeStr == "usz") {
    return "TypeKind::Int";
...
static bool isClassType(const String &typeStr,
...
static bool isAbstractClass(const ParsedClass &cls) {
  ...
  for (usz i = 0; i < cls.parentClasses.size(); ++i) {
    if (cls.parentClasses[i] == "TreeItem" ||
        cls.parentClasses[i] == "Collection::TreeItem" ||
        cls.parentClasses[i] == "public TreeItem") {
...
static String generateMetadataBlock(const Array<ParsedClass> &classes) {
  ...
  for (usz i = 0; i < classes.size(); ++i) {
    ...
    if (cls.name.indexOf('<') >= 0)
      continue; // skip template classes

    String cleanName = sanitizeName(cls.name);

    // 1. Static wrappers for each method to support generic reflection
    // invocation
    for (usz m = 0; m < cls.methods.size(); ++m) {
      ...
      out += "inline void* " + cleanName + "_" + method.name + "_" +
             String((long long)m) + "_wrapper(void* inst, void* args) {\n";
      if (!method.isStatic) {
        out += "    if (inst) {\n";
        out += "        auto* over = "
               "Sew::Reflect::ReflectionRegistry::getOverride((usz)inst, \"" +
               method.name + "\");\n";
    ...
    out +=
        "inline const StructDescriptor& get_" + cleanName + "_descriptor() {\n";
    ...
    if (cls.name == "Log" || cls.name == "Xi::Log" ||
        cls.name == "Graphics::Screen" || cls.name == "Screen") {
    ...
    out +=
        "        d.destroy = [](void* p) { delete (" + cls.name + "*)p; };\n";
    for (usz p = 0; p < cls.parentClasses.size(); ++p) {
      String parentName = cls.parentClasses[p];
      String fullName = parentName;

      // Resolve the fully-qualified parent name (e.g. "Transform" ->
      // "Xi::Transform")
      for (usz cIdx = 0; cIdx < classes.size(); ++cIdx) {
        long long lastColon = -1;
        for (usz k = 0; k + 1 < classes[cIdx].name.length(); ++k) {
          if (classes[cIdx].name.data()[k] == ':' &&
              classes[cIdx].name.data()[k + 1] == ':') {
            lastColon = (long long)k;
          }
        }
        String shortName =
            (lastColon >= 0) ? classes[cIdx].name.substring((usz)lastColon + 2)
                             : classes[cIdx].name;
        if (shortName == parentName) {
          fullName = classes[cIdx].name;
          break;
        }
      }
      out += "        d.parentClasses.push(\"" + fullName + "\");\n";
    }
    ...
    for (usz f = 0; f < cls.fields.size(); ++f) {
      ...
      if (field.name == "operator" || field.name == "const" ||
          field.name == "static" || field.name == "class" ||
          field.name == "struct" || field.name == "template" ||
          field.name == "public" || field.name == "private" ||
      ...
      // Resolve the fully-qualified field type name (e.g. "Vector3" ->
      // "Xi::Vector3")
      String fullCustomTypeName = customTypeName;
      for (usz cIdx = 0; cIdx < classes.size(); ++cIdx) {
        long long lastColon = -1;
        for (usz k = 0; k + 1 < classes[cIdx].name.length(); ++k) {
          if (classes[cIdx].name.data()[k] == ':' &&
        ...
        String shortName =
            (lastColon >= 0) ? classes[cIdx].name.substring((usz)lastColon + 2)
                             : classes[cIdx].name;
        if (shortName == customTypeName) {
          fullCustomTypeName = classes[cIdx].name;
          break;
        }
      }
      ...
      out += "            fd.kind = " + kindStr + ";\n";
      out += "            fd.offset = offsetof(" + cls.name + ", " +
             field.name + ");\n";
      out += "            fd.size = sizeof(((" + cls.name + "*)0)->" +
             field.name + ");\n";
      out += "            fd.customTypeName = \"" + fullCustomTypeName +
             "\";\n"; // <-- Updated to use fully-qualified name
    ...
    for (usz m = 0; m < cls.methods.size(); ++m) {
      ...
      out += "            md.functionPtr = (void*)&" + cleanName + "_" +
             method.name + "_" + String((long long)m) + "_wrapper;\n";
    ...
    out += "        ReflectionRegistry::registerStruct(get_" + cleanName +
           "_descriptor());\n";
    out += "        global[\"" + cls.name + "\"] = \"" + cls.name + "\";\n";
    out += "    }\n";
    out += "};\n";
    out += "inline Register_" + cleanName + "_Helper s_reg_" + cleanName +
           "_helper;\n\n";
  }
...
static inline bool matchesAtIndex(const String &source, usz index,
                                  const String &target) {
...
static Array<DetectedVar> detectGlobalVars(const String &source,
                                           const Array<ParsedClass> &classes) {
  ...
  while (i < source.length()) {
    ...
    if (!inComment && !inLineComment && !inString) {
      if (c == 'R' && i + 2 < source.length() && source[i + 1] == '"' &&
          source[i + 2] == '(') {
        i += 3;
        while (i < source.length()) {
    ...
    if (braceDepth == 0) {
      for (usz ci = 0; ci < classes.size(); ++ci) {
        ...
        if (className.isEmpty() || className.length() == 0)
          continue;
        ...
        if (matchesAtIndex(source, i, className)) {
          bool boundaryBefore =
              (i == 0 || !isAlnum(source[i - 1]) && source[i - 1] != '_');
          usz origEnd = i + className.length();
          bool boundaryAfter =
              (origEnd >= source.length() ||
               !isAlnum(source[origEnd]) && source[origEnd] != '_');
          ...
          if (boundaryBefore && boundaryAfter) {
            ...
            while (nextPos < source.length() &&
                   (isAlnum(source[nextPos]) || source[nextPos] == '_')) {
              nextPos++;
            }
            String varName = source.substring(idStart, nextPos);
            bool isGlobal = false;
            if (varName.length() > 0 && varName != "class" &&
                varName != "struct" && varName != "enum" &&
                varName != "union" && varName != "const") {
...
static String rewriteCastsAndIdentifiers(const String &source,
                                         const Array<ParsedClass> &classes,
                                         const Array<DetectedVar> &globalVars) {
  String rewritten;
  int braceDepth = 0;
  bool inComment = false;
  bool inLineComment = false;
  ...
  while (i < source.length()) {
    ...
    // ─── C++ compile-time refTypeName(Type) compiler-intercept ───
    if (!inComment && !inLineComment && !inString) {
      if (matchesAtIndex(source, i, "refTypeName")) {
        usz openParenIdx = i + 8;
        while (openParenIdx < source.length() &&
               isSpace(source[openParenIdx])) {
          openParenIdx++;
        }
        if (openParenIdx < source.length() && source[openParenIdx] == '(') {
          usz closeParenIdx = openParenIdx + 1;
          int parenL = 1;
          while (closeParenIdx < source.length() && parenL > 0) {
            if (source[closeParenIdx] == '(')
              parenL++;
            else if (source[closeParenIdx] == ')')
              parenL--;
            closeParenIdx++;
          }
          if (parenL == 0) {
            // Extract the raw type inside the parentheses
            String rawType =
                source.substring(openParenIdx + 1, closeParenIdx - 1).trim();

            // Strip all spaces for absolute 100% binary string compatibility
            String cleanType;
            for (usz k = 0; k < rawType.length(); ++k) {
              if (!isSpace(rawType[k])) {
                cleanType.push(rawType[k]);
              }
            }

            // Replace with dynamic C++ String construction
            rewritten += "Collection::String(\"" + cleanType + "\")";
            i = closeParenIdx;
            continue;
          }
        }
    ...
    for (usz ci = 0; ci < classes.size(); ++ci) {
      ...
      if (matchesAtIndex(source, i, "static_cast<")) {
        ...
        if (matchesAtIndex(source, classStart, className) &&
          ...
          rewritten += "Sew::Reflect::ReflectionRegistry::resolveCast<" +
                       className + "*>";
    ...
    for (usz vi = 0; vi < globalVars.size(); ++vi) {
      ...
      if (matchesAtIndex(source, i, name)) {
        ...
        if (boundaryBefore && boundaryAfter) {
          if (name.length() == 0) {
            ::printf("\n[ERROR] Infinite loop prevented! Empty variable name "
                     "of type '%s' matched.\n",
                     type.c_str());
            ::fflush(stdout);
            ::exit(1);
          ...
          if (braceDepth > 0) {
            ...
            while (prev >= 0 && isSpace(source[(usz)prev]))
              prev--;
            long long tokEnd = prev + 1;
            while (prev >= 0 &&
                   (isAlnum(source[(usz)prev]) || source[(usz)prev] == '_'))
              prev--;
            String prevTok = source.substring((usz)prev + 1, (usz)tokEnd);
            if (prevTok.length() > 0 && prevTok != "return" &&
                prevTok != "goto" && prevTok != "throw" && prevTok != "case" &&
                prevTok != "new" && prevTok != "delete") {
              isDeclaration = true;
              shadows[vi].depth++;
          ...
          while (pidx >= 0 && isSpace(source[(usz)pidx]))
            pidx--;
          if (pidx >= 0) {
            if (source[(usz)pidx] == '.') {
              isMemberAccess = true;
            } else if (pidx >= 1 && source[(usz)pidx] == '>' &&
                       source[(usz)pidx - 1] == '-') {
              isMemberAccess = true;
            } else if (pidx >= 1 && source[(usz)pidx] == ':' &&
                       source[(usz)pidx - 1] == ':') {
...
static void collectParentClassesRecursive(const ParsedClass &cls,
  ...
  for (usz p = 0; p < cls.parentClasses.size(); ++p) {
    ...
    for (usz j = 0; j < classes.size(); ++j) {
      ...
      if (!match) {
        long long lastColon = -1;
        for (usz k = 0; k + 1 < classes[j].name.length(); ++k) {
          if (classes[j].name.data()[k] == ':' &&
              classes[j].name.data()[k + 1] == ':') {
            lastColon = (long long)k;
          }
        }
        String shortName = (lastColon >= 0)
                               ? classes[j].name.substring((usz)lastColon + 2)
                               : classes[j].name;
        if (shortName == parentName) {
          match = true;
        }
...
static void collectFieldTypesRecursive(const ParsedClass &cls,
  ...
  for (usz f = 0; f < cls.fields.size(); ++f) {
    ...
    if (cleanTypeName.endsWith("&"))
      cleanTypeName =
          cleanTypeName.substring(0, cleanTypeName.length() - 1).trim();
    if (cleanTypeName.endsWith("*"))
      cleanTypeName =
          cleanTypeName.substring(0, cleanTypeName.length() - 1).trim();
...
static String rewriteCppSource(const String &source,
  ...
  for (long long j = (long long)globalVars.size() - 1; j >= 0; --j) {
    ...
    content = content.substring(0, var.declStart) + var.name + "_raw" +
              content.substring(var.declEnd);

    String helper;
    helper += "\n";
    helper += "struct Register_" + var.name + "_Helper {\n";
    helper += "    Register_" + var.name + "_Helper() {\n";
    helper += "        Sew::Reflect::ReflectionRegistry::registerVariable(\"" +
              var.name + "\", &" + var.name + "_raw);\n";
    helper += "    }\n";
    helper += "};\n";
    helper += "inline Register_" + var.name + "_Helper s_reg_" + var.name +
              "_helper;\n";

    usz adjustedInsertPos = var.insertPos + 4; // account for "_raw" suffix
    content = content.substring(0, adjustedInsertPos) + helper +
              content.substring(adjustedInsertPos);
  }
  ...
  // Second, recursively collect all parent classes of those matched classes
...
Array<ImportSpec> CppLanguage::parseImports(const String &source,
  ...
  if (!skipSiblings) {
    ...
    // Sibling source files for the header itself (e.g. String.cpp for
    // String.hpp)
...
CompileResult CppLanguage::compile(const CompileRequest &req) {
  ...
  if (req.sourcePath.endsWith("Reflection.cpp") ||
      req.sourcePath.endsWith("sew_bridge.cpp")) {
    CompileRequest modReq = req;
    ...
    // Safe temporary array to collect extra paths without modifying
    // req.includePaths in-place
  ...
  // --- Check if it is a third-party library source file ---
  bool isThirdParty = req.sourcePath.includes("/deps/") ||
                      req.sourcePath.includes("/thirdparty/") ||
                      req.sourcePath.includes("/diligent/") ||
                      req.sourcePath.includes("/glfw/") ||
                      req.sourcePath.includes("/sew/include/") ||
                      req.sourcePath.includes("include/Languages/") ||
                      req.sourcePath.includes("include/Sew/");

  if (isThirdParty) {
    // Direct, fast compilation of raw third-party source code with NO
    // rewriting!
    CompileRequest modReq = req;
    ...
    return invokeClang(modReq, req.sourceContent);
  }

  // --- Normal project source files get fully rewritten and reflected ---
...
CompileResult CppLanguage::invokeClang(const CompileRequest &req,
  ...
  if (req.targetTriple.length() > 0) {
    ...
    // Automatically add sysroot for wasm targets on Fedora/RHEL if using system
    // clang++ and it's present
  ...
  if (xicPath) {
    ...
    const char *tryPaths[] = {"../xic/include", "/home/xi/Repo/xic/include",
                              nullptr};
  ...
  if (req.sourcePath.includes("sew_bridge") ||
      req.sourcePath.includes("sew_qjs_bindings")) {
    p.arg.push("-O0");
    p.arg.push("-g0");
  }
...
public:
  ...
  ~Optimizer();
  void
  SetMessageConsumer(std::function<void(spv_message_level_t, char const *,
                                        spv_position_t const &, char const *)>
                         consumer);
  void RegisterLegalizationPasses();
  void RegisterPerformancePasses();
  void RegisterPass(PassToken &&token);
  bool Run(const uint32_t *code, size_t size, std::vector<uint32_t> *optimized,
           spv_optimizer_options_t *options) const;
};
Optimizer::PassToken::~PassToken() {}
Optimizer::Optimizer(spv_target_env env) {}
Optimizer::~Optimizer() {}
void Optimizer::SetMessageConsumer(
    std::function<void(spv_message_level_t, char const *,
                       spv_position_t const &, char const *)>
        consumer) {}
...
bool Optimizer::Run(const uint32_t *code, size_t size,
...
void spvValidatorOptionsSetBeforeHlslLegalization(spv_validator_options_t *,
                                                  bool) {}
spv_optimizer_options_t *spvOptimizerOptionsCreate() { return nullptr; }
void spvOptimizerOptionsDestroy(spv_optimizer_options_t *) {}
void spvOptimizerOptionsSetValidatorOptions(spv_optimizer_options_t *,
...
namespace Diligent {
std::vector<uint32_t>
ConvertUBOToPushConstants(const std::vector<uint32_t> &SPIRV,
